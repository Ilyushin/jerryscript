 ...
 */
void vm_init(const vm_instr_t *program_p, /**< pointer to byte-code program */
 ...
 * Run global code
 */
jerry_completion_code_t vm_run_global(void) {
	JERRY_ASSERT(__program != NULL);
	...
	ecma_completion_value_t completion = vm_run_from_pos(__program, start_pos,
			ecma_make_object_value(glob_obj_p), lex_env_p, is_strict, false);

	jerry_completion_code_t ret_code;

	if (ecma_is_completion_value_return(completion)) {
		JERRY_ASSERT (ecma_is_value_undefined(
 ...
 */
ecma_completion_value_t vm_loop(vm_frame_ctx_t *frame_ctx_p, /**< interpreter context */
 ...
 *   or NULL - if there is no active run scope */
{
	...
	while (true) {
		do {
			...
			JERRY_ASSERT(
					!ecma_is_completion_value_normal(completion)
		...
		if (ecma_is_completion_value_jump(completion)) {
			vm_instr_counter_t target =
					ecma_get_jump_target_from_completion_value(completion);
 ...
 */
ecma_completion_value_t vm_run_from_pos(const vm_instr_t *instrs_p, /**< byte-code array */
 ...
 */
vm_instr_t vm_get_instr(const vm_instr_t *instrs_p, /**< byte-code array */
 ...
 */
opcode_scope_code_flags_t vm_get_scope_flags(const vm_instr_t *instrs_p, /**< byte-code array */
vm_instr_counter_t counter) /**< instruction counter */
{
	vm_instr_t flags_instr = vm_get_instr(instrs_p, counter);
	JERRY_ASSERT(
			flags_instr.op_idx == VM_OP_META
					&& flags_instr.data.meta.type
							== OPCODE_META_TYPE_SCOPE_CODE_FLAGS);
